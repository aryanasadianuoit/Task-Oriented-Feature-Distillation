...
SERVER_1_PREFIX_PATH = "/home/aasadian/saved/"
SERVER_1_PREFIX_PATH_SYSTEMATIC = "/home/aasadian/saved/systematic/"
SERVER_2_PREFIX_PATH = "/home/aasadian/virtualvenvs/gputestvenv/fitnes_from_scratch/saved/"
SERVER_2_GRID_PREFIX_PATH = "/home/aasadian/virtualvenvs/gputestvenv/fitnes_from_scratch/codistillation/grid/"
SERVER_2_PREFIX_PATH_CODISTILLATION = "/home/aasadian/virtualvenvs/gputestvenv/fitnes_from_scratch/codistillation/"
...
def test_data_evaluation(model,
    ...
    if added_regressor != None:
        print("TODO Delete the regressor")
    ...
    if state_dict_loaded == False:
        ...
        #model1_path = "/home/aasadian/virtualvenvs/gputestvenv/fitnes_from_scratch/codistillation/modular_real/_layer_1_model.pth"
        #model2_path = "/home/aasadian/virtualvenvs/gputestvenv/fitnes_from_scratch/codistillation/modular_real/_layer_2_model.pth"
        #model3_path = "/home/aasadian/virtualvenvs/gputestvenv/fitnes_from_scratch/codistillation/modular_real/_layer_3_model.pth"
        #model3_path = "/home/aasadian/virtualvenvs/gputestvenv/fitnes_from_scratch/codistillation/modular_real/_layer_3_model.pth"
        #layer_1_model.load_state_dict(torch.load(model1_path))
        #model.attached_part.load_state_dict(torch.load(model1_path))
    since = time.time()
    model.eval()
...
def get_intermediate_ouput_dict(model,model_module,example_input_tensor = torch.rand((1,3,32,32))):
    ...
    def assign_hooker(target_module,key_value):
        def forward_hooker_internal(self, input, output):
            ...
            #if isinstance(target_module,torch.nn.Conv2d) or isinstance(target_module,torch.nn.Linear) or isinstance(target_module,torch.nn.ReLU):
                #intermediate_output_dict[key_value] = output.size()
                #print("Type ",type(target_module))
        target_module.register_forward_hook(forward_hooker_internal)
...
def gram_matrix(matrix_1,matrix_2):
    """
    :param matrix_1: First tensor( generaly a4-d Tensor(batch,channels,height,width)
    :param matrix_2: Second tensor(generaly a4-d Tensor(batch,channels,height,width)
    In our case, we consider identical input tensors
    :return: a list of gram matrices each member is the gram matrix for one channel. in the case of 64 channels, the
    output is a 64 member list which each of its member is  a (h,w) matrix
    //Todo used for multi dimensional approaches with averaging along the dimensions
    """
    gram_list = []
    #Assumption is matrix 1 & 2 have same dimensionality
    for channel in range((len(matrix_1[0]))):
        #print("HOOOOO")
        gram = matrix_1[0][channel] * matrix_2[0][channel]
        #print("No",len(matrix_1[0][0]))
        #print("Yes",len(matrix_1[0][1]))
        gram /= len(matrix_1[0][0]) * len(matrix_1[0][1])
...
def annealing_approach(parameter_initial = 0.1,paramter_max = 1, total_itterations = 200):
    epochs = 200
    pi_tensor = torch.tensor([math.pi], device="cuda")

    gamma_initial = 0.001
    gamm_max = 1.0
    for epoch in range(epochs):
        final_gamma = 0.5 * (1 + torch.cos(epoch / epochs * pi_tensor) * (gamma_initial - gamm_max))
        print("Epoch : ", epoch, "\t\t Final Gamma : ", final_gamma.data)
...
def get_mean_std(loader):
    ...
    #Note For computing meand and std the shuffleing for the train loader should be disabled(shuffle = False)


    #Inspired by the solution of alaadin
...
def get_mean_std_ptr(loader):

    #Note For computing meand and std the shuffleing for the train loader should be disabled(shuffle = False)

    #Copied from the Solution of PtrBlk :
    # https://discuss.pytorch.org/t/about-normalization-using-pre-trained-vgg16-networks/23560/6?u=aryan_asadian
    #(Normalization through the pixels of the input images
    #VAR[X] = E[X**2] -E[X]**2
    #std = (VAR)**0.5
    mean = 0.
    std = 0.
